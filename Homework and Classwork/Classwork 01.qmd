...
Monte Carlo simulations are useful for understanding how models behave because they give us a straightforward way to judge performance: we made the data, so we know the "true" values of our parameters, and so we can easily calculate things like the bias or variance of an estimator under a whole range of hypothetical scenarios - including cases w
...
You can try modifying various parameters to see what changes in the results here. For instance, we could increase the sample size from 500 to 5000 and we should expect the estimates to get closer to the correct value:
...
for(i in 1:iterations){
  ...
  Y <- beta_0 + X * beta_1 + e # the dependent variable
  mod<- lm(Y ~ X)
  coefs[[i]]<- tidy(mod, conf.int = TRUE) # make a data frame from the model
}
...
# keeping iterations, random seed and sample size the same:
set.seed(999)
iterations<-1000
N <-  500 
...
```{r}
betas <- c(1, 3) # Make a vector of betas
corr_x <- .8
intercept <- 1
Sigma <- matrix(c(1, corr_x, corr_x, 1), ncol=2)
X <- MASS::mvrnorm(N, mu=c(0, 0), Sigma=Sigma)
e <- rnorm(N, 0 , sd=1) # the variance of the error term
...
betas <- c(1, 3) # Make a vector of betas
intercept <- 1
correlations <- c(-.8, 0, .8)
results<-tibble()

for(ci in correlations) {
  coefs<-list()
  for (i in 1:iterations) {
    Sigma <- matrix(c(1, ci, ci, 1), ncol = 2)
    X <- MASS::mvrnorm(N, mu = c(0, 0), Sigma = Sigma)
    e <- rnorm(N, 0 , sd = 1) # the variance of the error term
    Y <- intercept  + X %*% betas + e
    mod <- lm(Y ~ X[, 1]) # Omitting X[,2] from the regression model
  ...
  coefmat<-bind_rows(coefs, .id='iteration')
  coefmat$correlation<-paste("r = ", ci)

  results<-bind_rows(results, coefmat)
...
How much do our estimates differ from the correct answer? And how does the relationship between these variables change the results?
...
results |> 
  ...
  ggplot(aes(x = bias, y = correlation, )) +
  stat_halfeye(
    alpha = .5,
    width = .6,
    .width = 0,
...
Rewrite the code above so that the effect of $\beta_2$ (the omitted variable) is negative instead of positive. How does this impact your results?
...
# Extra code: Using a simulation function
...
This amounts to asking how frequently the p values are greater than or equal to .05:
...
We already stored the estimated confidence interval for each result, so we just need to check how many confidence intervals do not contain 0.1:

```{r}
# re-using coefmat from above
...
This is equivalent to asking how frequently the p values are less than .05 across our simulated results
...
Flipping the sign should flip the effect of the omitted variable (a negative effect with a negative correlation will result in a positive bias)
