...
Note that we don't automatically get standard errors for these estimates. There are a number of methods for getting confidence intervals around a random effect. The `estimate_grouplevels` from the [modelbased](https://easystats.github.io/modelbased/index.html) package is an easy one:

```{r}

estimate_grouplevel(model_2)
