...
We'll try estimating a Bayesian version of the same model:



```{r}
library(rstanarm)
bayesian_model<-stan_glm(outcome ~ tactic_type, data=navco,   
...
We can use the `avg_comparisons` functions again here to get a marginal effect estimate:
...
The `get_draws` function will get simulated effects by randomly sampling from the posterior distribution from the model. 

```{r}
get_draws(mfx)|>
  ggplot(aes(x=draw, fill=contrast)) +
    stat_halfeye(slab_alpha = .5) +
    labs(x = "Marginal Effect of tactics",
         y = "Posterior density",
         fill = "tactics") +
  theme_bw()
...
Or we can visualize predictions for each category:

```{r}
preds<-avg_predictions(bayesian_model, variables='tactic_type')


get_draws(preds)|>
  ggplot(aes(x=draw, fill=tactic_type)) +
    stat_halfeye(slab_alpha = .5) +
    labs(x = "Predicted probability of success",
         y = "Posterior density",
         fill = "tactics") +
  theme_bw()
...
We can also try specifying different values for our priors. (The defaults in `rstanarm` frequently work very well, but sometimes you might want to modify them, especially if you have some prior research that gives you a reason to use a prior centered on some value other than zero)



```{r}

bayesian_model2<-stan_glm(outcome ~ tactic_type, data=navco,   
                       family = binomial(link = "logit"),
                       seed = 999, 
                       refresh = 0 ,
...
The results here don't change too dramatically even when we use a different prior, likely because the model is relatively simple and, at this sample size, the data is far more informative than the prior. 
