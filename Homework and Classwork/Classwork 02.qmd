...
We can use the same general approach to explore the effects of multicollinearity. 
...
genFunc <- function(corr_x=0, e_sd=1, N=1000, betas=c(1,1)){
  ...
  e = rnorm(N, 0 , sd=e_sd) # the variance of the error term
  Y <- intercept  + X %*% betas + e
  mod <- lm(Y ~ X) # Omitting X[,2] from the regression model
  coefs<-tidy(mod, conf.int = TRUE, conf.level = .95)
  return(coefs)
...
Now we can see how our model performs with differing levels of multicollinearity.
...
results |> 
  ...
  ggtitle("Effects of multicollinearity")
...
How about the effects on Type I error rates?

```{r}

plist<-list(
  'r=-.7' =list(corr_x =  0.7, betas=c(0,1)),
  'r=  8' =list(corr_x =  0.8, betas=c(0,1)),
  'r= .9' =list(corr_x =  0.9, betas=c(0,1))
)
...
results|>
  filter(term == 'X1')|>
  summarise(type1 = mean(p.value<=.05))
...
checked<-check_heteroscedasticity(model)
...
checked<-check_heteroscedasticity(model2)
...
genFunc <- function(N=1000, homoskedastic=FALSE){
  betas <- 0
  X = cbind(runif(N, 1, 10))
  intercept = 1
  var =ifelse(homoskedastic, 1, exp(X))
  e = rnorm(N, 0 , var)
  Y <- intercept  + X %*% betas + e
  mod <- lm(Y ~ X)
  coefs<-tidy(mod, conf.int = TRUE, conf.level = .95)
...
plist<-list(
  'N=20 homoskedastic' = list(N=25, homoskedastic=TRUE),
  'N=20 heteroskedastic' =  list(N=25),
  'N=100 homoskedastic' = list(N=100, homoskedastic=TRUE),
...
results|>
  ...
  geom_boxplot(width = .15, outlier.shape = 3) +
  theme_bw() +
...
results|>
  filter(term=="X")|>
  group_by(condition)|>
  summarize(simulated_sd = sd(estimate),
            estimated_se = median(std.error)
         )
