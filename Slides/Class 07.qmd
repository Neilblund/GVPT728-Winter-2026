...
```{r, echo=FALSE}
vioname <- c("VIOL" = "Primarily violent")
...
navco_lpm<-lm(SUCCESS ~  VIOL, 
                 data=navco)

navco_logit<-glm(SUCCESS ~  VIOL, 
                 data=navco, 
                 family=binomial(link='logit'))

navco_probit<-glm(SUCCESS ~ VIOL, 
                  data=navco,
...
#  Pr(Y=1) at viol = 0 
pr<-mean(navco$SUCCESS[which(navco$tactic =='Violence')])
pr
...
mean(navco$SUCCESS[which(navco$tactic=="Non-violence")])
