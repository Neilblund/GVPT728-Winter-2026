...
beta_1<- 2.5 # the effect of X on Y
result_list<-list()

conditions<-list('0% missing data' = 1,
                 '25% missing data' = .75,
                 '50% missing data' = .5,
                 '75% missing data' = .25
                 )
...
for(i in 1:iterations){
  ...
  Y <- beta_0 + X * beta_1 + e 
  data<-data.frame(Y, X)
  condition_frame<-list()
  for(j in 1:length(conditions)){
    kept_data<-round(nrow(data)  * conditions[[j]])
    data_missing <- data[sample(nrow(data), size=kept_data),]
    mod<- lm(Y ~ X, data=data_missing)
    condition_name <- names(conditions)[j]
    condition_frame[[condition_name]] <- tidy(mod, conf.int = TRUE)

  }
  condition_frame<-bind_rows(condition_frame, .id='condition')
  result_list[[i]] <- condition_frame

}

result_frame <- bind_rows(result_list, .id='iteration')
result_frame|>
  filter(term == "X")|>
  ...
  theme_bw() +
  labs(title = "Estimated coefficients under MCAR") +
...
All these cases suggest that we can fully account for the characteristic that causes missing data using observable data. We have data on wealth for the global development data, we have data on member's age from our congress study, and we can identify who belongs to vulnerable groups.
...
betas <- c(2.5, -10)
result_list<-list()
conditions<-list('Y ~ X1 + X2' = formula(Y~X1 +X2),
                 'Y ~ X1' = formula(Y~X1)
)
for(i in 1:iterations){
  ...
  condition_frame<-list()
  for(j in 1:length(conditions)){
    keep_index<-which(data$X2 >= quantile(data$X2, .75))

    data_missing <- data[keep_index,]
    
    
    mod<- lm(conditions[[j]], data=data_missing)
    condition_name <- names(conditions)[j]
    condition_frame[[condition_name]] <- tidy(mod, conf.int = TRUE)
...
result_frame|>
  ...
  geom_boxplot(width = .15, outlier.shape = NA) +
  theme_bw() +
  labs(title = "Estimated coefficients under MAR") +
  geom_vline(xintercept = 2.5)
...
We don't really even have partial data on the cause of missingness here: we might suspect that "low potential income" cause someone to decide not to enter the labor force, but we never observe wages for that person in the first place so this is potentially un-testable.
...
for(i in 1:iterations){
  X<-rnorm(N)
  y_true <- 1 + X * 4 + rnorm(N)
  prob_missing <- plogis(y_true - mean(y_true)) 
  r <- rbinom(length(y_true), size = 1, prob = prob_missing)
    y_observed <- y_true
  y_observed[r == 1] <- NA
  ...
  mod<-lm(y_observed~ X, data=data_mnar)
  result_list[[i]]<-tidy(mod)
  
}
result_frame <- bind_rows(result_list, .id='iteration')
result_frame|>
  filter(term == "X")|>
  ggplot(aes(x =estimate, y ="MNAR")) +
  stat_halfeye(
    alpha = .5,
    justification = -.2,
    width = .6,
  ...
  geom_boxplot(width = .15, outlier.shape = NA) +
  theme_bw() +
  labs(title = "Estimated coefficients under MNAR") +
...
-   For all missing data: less is better. If there are only a handful of missing cases, then its unlikely that missingness has a huge effect on the results.
