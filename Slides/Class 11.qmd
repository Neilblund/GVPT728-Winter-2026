...
Bad weather decreases protest turnout, but it should be uncorrelated with public or elite opinion.
...
The instrument is correlated with our "treatment" of protest turnout, but its not a perfect correlation because some people protest in bad weather. So how does this impact our results? This can be analogous to a situation that occurs in true randomized experiments.
...
::::: columns
::: {.column width="50%"}
-   What if non-compliance is *caused by* the intent to treat itself? For instance: what if people knocking at your door, rather than actually speaking to a canvasser, causes you to go vote?
...
So, this strategy only works if we can be sure that there are no defiers.
...
::: incremental
1.  Relevance: The instrument must have an effect on the causal variable of interest. The stronger the better. 
2.  Exogeneity: The instrument should be random or conditionally random with respect to the causal variable and the outcome
...
digraph {
    graph [rankdir=TB]
    node [shape =oval style=dashed]
    U[label="Conservatism"]
    node [shape=box, height=0.3, width=0.3, style=solid]
    D[label="Fox Viewership"]
    Y[label="Social distancing"]
    D -> Y [minlen=4]
    U -> D, Y 
    { rank = same; D Y }
}
...
- Channel position in the cable lineup is *correlated* with viewership.

- Channel positions are probably not correlated with the potential confounders in the Fox News viewership DAG.


:::
::: {.column width="50%"}
![](images/fnc_position.png)
...
- If channel position is exogenous, then it only impacts social distancing through its effect on Fox viewership.
...
- A one standard deviation increase in FNC position leads to about 11 extra minutes per week of viewership

- This increase is also associated with an average of:
  - 3.5 extra minutes outside per day
  - 0.3 extra km per day, and
  - \$106 in Covid related spending per week.


:::
::: {.column width="50%"}
![](images/urn_cambridge.org_id_binary_20240305114907928-0029_S1047198723000219_S1047198723000219_fig1.png)
...
- Claim: average national-level IQ is a cause of economic growth.

:::
::: {.column width="50%"}
![](images/mankind_quarterly.png)
:::
:::::
...
- IQ is confounded because growth also makes people smarter.

- You can't really "control" for this kind of confounding, because the confounder is the dependent variable itself.
...
::: {.nonincremental}
- The authors used several instrumental variables models to attempt to bolster the argument, including average UV radiation in 1500.
...
- The problem is that UV radiation is probably not: 
  - Relevant: because there's not much evidence to link it to IQ.
  - Exogenous: because UV radiation is directly and indirectly linked to things like labor productivity, agricultural output, colonization etc that cause growth.
