...
-   "Units" must be observed before and after the "treatment", so most commonly applied to panel data (although it's also viable for time series)
...
If we can assume a parallel trend, then the relationship should look like this. The effect size, then, would be the difference between the counter factual case and the observed case.
...
$B_0$ The average for the control group at $\text{Time}=0$

</li>

<li class="fragment">

$B_1$ The average for the control group at $\text{Time}=1$

</li>

<li class="fragment">

$B_2$ The difference between the treated and control units at $\text{Time}=0$

</li>

<li class="fragment">

$B_3$ The difference in slopes for the treated group compared to the control group $\text{Time}=1$
...
:::: {.column width="50%"}
::: non-incremental
-   If we assume parallel trends, then the slope for New Jersey would be the same as the slope for Pennsylvania, so we'd expect their average employment to be $20.44 - 2.16 = 18.28$
:::
::::
...
ggplot(aes(x=time_period, y=avg_employment, group=state,
  ...
  annotate("text" , x=1.9, y=23,1, label=as.character(TeX("$21.17 - 23.33  = -2.16$")), parse=TRUE) +
  annotate('text', x=1.9, y=19.85, label=as.character(TeX("$20.44 - 2.16 = 18.28$")), parse=TRUE) +
...
-   New Jersey's actual average employment was $21.03$, so the difference-in-differences is $21.03 - 18.28 = 2.75$
:::
::::
...
ggplot(data, aes(x=time_period, y=avg_employment, group=state,
  ...
  expand_limits(x=c(1,4)) +
    annotate("text" , x=1.9, y=23.1, label=as.character(TeX("$21.17 - 23.33  = -2.16$")), parse=TRUE) +
    #annotate('text', x=1.9, y=16.85, label=as.character(TeX("$21.03 - 20.44 = 0.59 $")), parse=TRUE) +
    annotate('text', x=1.9, y=19.85, label=as.character(TeX("$20.44 - 2.16 = 18.28$")), parse=TRUE) +
    annotate('text' , x=3, y=21.03, label=as.character(TeX("DiD $= 21.03 - 18.28 = 2.75")), parse=TRUE)+
...
- (Note that including wave 2 characteristics wouldn't work here, because those could be the result of treatment.)
:::

::: {.column width="50%"}
![Card, D. (1993). Minimum Wages and Employment: A Case Study of the Fast Food Industry in New Jersey and Pennsylvania.](images/card_and_krueger_table5.png){width="80%"}
:::
:::::

## Card and Krueger 1994

[Card and Krueger replication script](https://raw.githubusercontent.com/Neilblund/GVPT728-Winter-2026/refs/heads/main/Additional%20Code%20and%20Data/ck_did_example.R)
