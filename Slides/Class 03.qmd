...
voteshares<-left_join(voteshares, acs_data, by=join_by(state==NAME))
...
model_1 <-lm(demshare ~ income_over_65k, data=voteshares)
obs_effect<-model_1$coefficients[2]
effects<-readRDS('Slide data/effects.rds')
mean((effects >= obs_effect) | (effects <= obs_effect * -1)) * 100
...
data.frame(effects)|>
  ...
  geom_histogram() +
  geom_vline(xintercept = obs_effect, col='red',lty=2) +
  geom_vline(xintercept = obs_effect *-1, col='red',lty=2) +

  theme_bw() +
  labs(x = "Estimated effects under sharp null") 
...
data.frame(ub = difference + (sdpooled * sqrt(1/n_0 + 1/n_1)) * qnorm(.95),
           lb= difference - (sdpooled * sqrt(1/n_0 + 1/n_1)) * qnorm(.95),
           est = difference,
           ueq = .48,
           leq = -.48
  ...
  ggplot(aes(x=est, y="")) + 
  geom_pointrange(aes(xmin = ub, xmax=lb)) + geom_point() +
  geom_vline(aes(xintercept=ueq), col='red', lty=2) +
  geom_vline(aes(xintercept=leq), col='red', lty=2) +
...
![](images/rainey_plot.jpg){fig-align="center"}
...
voteshares$pct_bachelors <- (voteshares$bachelors_degree / voteshares$pop) * 100

model <-lm(demshare ~  poly(pct_bachelors, 2, raw=T), data=voteshares)
model|>summary()


df<-data.frame(preds = (predict(model, newdata=data.frame(pct_bachelors = seq(0, 100, by=1)
                                                          ))),
               pct_bachelors = seq(0, 100, by=1))


ggplot(df, aes(x=pct_bachelors, y=preds)) + geom_line() +
  theme_bw()
...
for(i in 1:nboot){
  booted<-voteshares|>slice_sample(n=nrow(voteshares), replace=T)
  boot_model <-lm(model$call, data=booted)
  obs<- predict(boot_model)
  higher<-predict(boot_model, newdata = booted|>mutate(pct_bachelors = pct_bachelors + 1) )
  diff<-mean(higher) - mean(obs)
  fx[i] <- diff
...
Another option is to use simulations:

```{r}
set.seed(999)
nreps<-1000
coefs <- mvtnorm::rmvnorm(nreps, coef(model), vcov(model))  #simulate random coefficients

model_data<-model.matrix(formula(model), data=model$model)
est<-coefs%*%t(model_data)

model_data_obs<-model.matrix(formula(model), data=voteshares)
model_data_high<-model.matrix(formula(model), data=voteshares|>
                                mutate(pct_bachelors = pct_bachelors + 1))

est0<-coefs%*%t(model_data_obs)

est1<-coefs%*%t(model_data_high)
Xdiff<-rowMeans(est1) - rowMeans(est0)

quantile(Xdiff, c(.025, .5, .975))
...
The [marginal effects](https://marginaleffects.com/) package will do this automatically:
...
avg_comparisons(model, variables='pct_bachelors', method='simulation')
...
avg_comparisons(model, variables='pct_bachelors', method='boot')
